...
(function () {
  ...
  function mount({ frameSelector } = {}) {
    ...
    wireResizer(frame);
    // Wire the bridge ONLY when the iframe's document is the real body
    // (not the browser's initial about:blank). At DOMContentLoaded of the
    // chrome, contentDocument is non-null but refers to about:blank; the
    // iframe is about to navigate to the body file, replacing that document
    // and discarding any listener we attach now. Wire immediately only when
    // the body has already loaded; otherwise wait for the load event.
    const doc = frame.contentDocument;
