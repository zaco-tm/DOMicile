...
- **74-second demo video on the GitHub Pages landing page and the README "See it in action" section.** Source was a 68 MB VP8/Vorbis WebM capture (1920×1080, 74 s); re-encoded with H.264 + AAC at ~1.3 Mbps to a 12.2 MB MP4 (`DomicileDemo.mp4`) and a 150 KB first-frame JPG poster (`DomicileDemo-poster.jpg`) — visually lossless for screen content, well under GitHub's 50 MB soft warning. Both files live at the repo root (alongside `index.html` and `domi-mascot-200.png`); the root `index.html` embeds the video in its own "See it in action" section between the hero and the loop section, so the voiceover is the second thing visitors encounter (after the hero's title/subhead). The frosted/plum video frame matches the existing card language; the loop section's 4-step diagram follows as a visual summary. The README's "See it in action" section replaces its "coming soon" placeholder with the same `<video>` embed. The root files are maintained directly; `.github/workflows/publish-landing.yml` is now manual-only (its `push` trigger is disabled) so a future `templates/landing-page/**` change won't clobber the root.

### Changed

- **Cleared `templates/landing-page/`** (`index.html` + `README.md` + `domi-mascot-200.png`) in preparation for a rebuild. The live site (root `index.html` + `DomicileDemo.mp4` + `DomicileDemo-poster.jpg` + `domi-mascot-200.png`) is unaffected and continues to be the source of truth. `.github/workflows/publish-landing.yml` was already disabled in this commit, so the deletion can't be auto-undone by a CI sync.
